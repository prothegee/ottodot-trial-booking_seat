...
# class: safe
#
# Tests the flags and the terminal guard of scripts/test_integration.sh, without
# starting a container or running a single test.
#
# Every case that runs the subject ends inside a guard or inside the dry run.
# The guard cases point DATABASE_CONTAINER at a name nothing answers to, which
# is what a machine with no stack looks like from here, so the refusing path can
# be proved without taking anybody's stack down.
#
...
subject="$repository_root/scripts/test_integration.sh"
...
# Runs the subject against a stack that is not there, so it can only refuse or
# print its plan.
#
# Param:
...
run_flag_cases() {
    ...
    # The plan starts nothing, so it is printed before the guard rather than
    # behind it. A workflow being able to read it is worth more than symmetry.
    expect_run "the plan prints without a terminal" 0 "" --dry-run
}
...
run_guard_cases() {
    ...
    expect_run "the refusal names the flag that answers it" 2 "--yes"
}
...
run_source_cases() {
    ...
    #
    # Counted rather than found, because the step that raises a stack asks the
    # same question. Finding one occurrence would pass with the guard gone.
    expect_count_in_code "the guard and the step that raises a stack both ask" \
