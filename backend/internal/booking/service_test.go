...
func (spy *spyRepository) LiveBooking(_ context.Context, _ string, _ string) (booking.Booking, error) {
    return booking.Booking{}, booking.ErrBookingNotFound
}

func (spy *spyRepository) Fail(_ context.Context, request booking.FailRequest) (booking.Booking, error) {
    spy.declines = append(spy.declines, request)

    return booking.Booking{ID: request.BookingID, Status: booking.StatusPaymentFailed}, nil
}
